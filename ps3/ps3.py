...
def get_word_score(word, n):
    ...
    for e in word_lower:
        first_component += SCRABBLE_LETTER_VALUES[e]
...
def update_hand(hand, word):
    ...
    for e in word:
        if e in hand.keys() and hand[e] > 0:
            new_hand[e] -= 1
    ...
    * Returns the total score for the series of hands
    ...
    while hands_number > 0:
        ...
        if not substitute_used:
            isSubstitute = input("Would you like to substitute a letter? ")
            if isSubstitute.lower() == "yes" or isSubstitute.lower() == "y":
                substitued_letter = input("Which letter would your like to replace: ")
                hand = substitute_hand(hand, substitued_letter.lower())
                substitute_used = True
        
        hand_score = play_hand(hand, word_list)
       
        if not replay_used:
            isReplay = input("Would you like to replay the hand? ")
            if isReplay.lower() == "yes" or isReplay.lower() == "y":
                replay_hand_score = play_hand(hand, word_list)
                isReplay = True
                if hand_score < replay_hand_score:
                    hand_score = replay_hand_score
...
if __name__ == '__main__':
    ...
    play_game(word_list)
    # hand = {'a':2, 'b':1, 'c':1}
    # new_hand = substitute_hand(hand, "b")
    # print(new_hand)